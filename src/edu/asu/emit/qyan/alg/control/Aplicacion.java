...
public class Aplicacion {
		...
		for(int z = 0; z < 2; z++) {
			...
			for(int x = 0; x < 1; x++) {
				...
				for (int a=0; a < listaAbejas.size(); a++) {
					ArrayList<Idrequest> caminosTomados = new ArrayList<Idrequest>();
					GrafoMatriz graf = copiarGrafo(listaAbejas.get(a).getG());
					List<Request> deman = copiarDemanda(listaAbejas.get(a).getDemandas());
					Abeja abe = new Abeja(listaAbejas.get(a).getId(),deman , graf);
					int su = 0;
					...
					for(int i= 0; i < abe.getDemandas().size(); i++) {
						...
						if (res !=null) {
							...
							caminosTomados.add(identificador);
							...
							cont++;
							System.out.println("No se encontró camino posible.");
							Idrequest identificador = new Idrequest(i, null);
							caminosTomados.add(identificador);
						}  
					}
					//	System.out.println("abejaID:"+ abe.getId() + "apl:" + abe.getAPL() + "su:" + abe.getSU());

					List<Request> nuevadem = marcarCamino(abe.getDemandas(), caminosTomados);
                   
					abe.setContadorBloqueo(cont);
					abe.setDemandas(nuevadem);
			...
			ArrayList<String> auxCamino = new ArrayList();
	...
	public static List<Request> marcarCamino(List<Request> demanda, ArrayList<Idrequest> resp) {
		...
		for (int i = 0; i < demanda.size(); i++) {
			...
			int origen = demanda.get(i).getOrigen();
			int destino = demanda.get(i).getDestino();
			int fs = demanda.get(i).getFs();
			int id = demanda.get(i).getId();
			String cam = resp.get(i).getCamino();
			auxiliar.setOrigen(origen);
			auxiliar.setDestino(destino);
			auxiliar.setFs(fs);
			auxiliar.setCaminoElegido(cam);
			auxiliar.setId(id);
	...
	public static List<Request> copiarDemanda(List<Request> deman) {
		List<Request> demandas = new ArrayList<Request>();

		for (int i = 0; i < deman.size(); i++) {
			Request auxiliar = new Request();
			int origen = deman.get(i).getOrigen();
			int destino = deman.get(i).getDestino();
			int fs = deman.get(i).getFs();
			int id = deman.get(i).getId();
			auxiliar.setOrigen(origen);
			auxiliar.setDestino(destino);
			auxiliar.setFs(fs);
			auxiliar.setId(id);
